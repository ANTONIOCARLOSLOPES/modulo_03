class MiningTypesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @mining_type.save
        format.html { redirect_to mining_type_url(@mining_type), notice: "Tipo de Mineração Criada Com Sucesso!" }
  ...
  def update
    respond_to do |format|
      if @mining_type.update(mining_type_params)
        format.html { redirect_to mining_type_url(@mining_type), notice: "Tipo De Mineração Atualizada Com Sucesso!" }
